 ...
 */
export function composeTransform(f, g) { //funções com parâmetro dx e dy definidos
  return function composed(x,y){ //valores x e y fornecidos a chamar função composeTransform
    return g (...f(x,y)); //spread para tirar os resultados da função f do formato array
  }
}
...
export function memoizeTransform(f) {
  let xAnterior, yAnterior, resultadoAnterior;

  return function memoize(x,y){
    if (x===xAnterior && y===yAnterior) return resultadoAnterior;
    xAnterior = x;
    yAnterior = y;
    resultadoAnterior = f (x,y);
